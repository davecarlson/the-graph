...
module.exports.register = function(context) {
  ...
  TheGraph.Graph = React.createFactory(
    createReactClass({
      ...
      edgeStart: function(event) {
        ...
        if (
          this.state.edgePreview &&
          this.state.edgePreview.isIn !== event.detail.isIn
        ) {
          ...
          const { onValidateEdge } = this.props;

          let isValid = true;
          if (onValidateEdge) {
            isValid = onValidateEdge(halfEdge);
          }
      ...
      getComponentInfo: function(componentName) {
        console.error(
          "DEPRECATED: getComponentInfo() will be removed in next version of the-graph. Use 'library' in props instead"
        );
      ...
      updateIcon: function(nodeId, icon) {
        console.error(
          "DEPRECATED: updateIcon() will be removed in next version of the-graph. Pass nodeIcons through props instead"
        );
      ...
      render: function() {
        ...
        var nodes = graph.nodes.map(function(node) {
          ...
          if (!componentInfo) {
            throw new Error(
              "Component " + node.component + " is not in library"
            );
          }
          var key = node.id;
          if (!node.metadata) {
          ...
          if (TheGraph.config.autoSizeNode && componentInfo) {
            ...
            var portCount = Math.max(
              componentInfo.inports.length,
              componentInfo.outports.length
            );
            if (portCount > TheGraph.config.maxPortCount) {
              var diff = portCount - TheGraph.config.maxPortCount;
              node.metadata.height =
                TheGraph.config.nodeHeight +
                diff * TheGraph.config.nodeHeightIncrement;
            }
          }
          ...
          return TheGraph.factories.graph.createGraphNode.call(
            this,
            nodeOptions
          );
        });
        ...
        var edges = graph.edges.map(function(edge) {
          ...
          var sourcePort = self.getNodeOutport(
            graph,
            edge.from.node,
            edge.from.port,
            route,
          ...
          var targetPort = self.getNodeInport(
            graph,
            edge.to.node,
            edge.to.port,
            route,
          ...
          return TheGraph.factories.graph.createGraphEdge.call(
            this,
            edgeOptions
          );
        });
        ...
        var inports = Object.keys(graph.inports).map(function(key) {
          ...
          if (!portInfo) {
            console.warn(
              "Node " + nodeKey + " not found for graph inport " + label
            );
            return;
          }
          ...
          edges.unshift(
            TheGraph.factories.graph.createGraphEdge.call(this, expEdge)
          );
        ...
        var outports = Object.keys(graph.outports).map(function(key) {
          ...
          if (!portInfo) {
            console.warn(
              "Node " + nodeKey + " not found for graph outport " + label
            );
            return;
          }
        ...
        var groups = graph.groups.map(function(group) {
          ...
          groupOptions = TheGraph.merge(
            TheGraph.config.graph.nodeGroup,
            groupOptions
          );
          return TheGraph.factories.graph.createGraphGroup.call(
            this,
            groupOptions
          );
        });
        ...
        if (this.state.displaySelectionGroup && selectedIds.length >= 2) {
          ...
          if (limits) {
            ...
            selectionGroupOptions = TheGraph.merge(
              TheGraph.config.graph.selectionGroup,
              selectionGroupOptions
            );
            var selectionGroup = TheGraph.factories.graph.createGraphGroup.call(
              this,
              selectionGroupOptions
            );
        ...
        if (edgePreview) {
          ...
          if (edgePreview.from) {
            ...
            var sourcePort = this.getNodeOutport(
              graph,
              edgePreview.from.process,
              edgePreview.from.port
            );
            edgePreviewOptions = {
              sX: source.metadata.x + source.metadata.width,
              sY: source.metadata.y + sourcePort.y,
            ...
            var targetPort = this.getNodeInport(
              graph,
              edgePreview.to.process,
              edgePreview.to.port
            );
            edgePreviewOptions = {
          ...
          edgePreviewOptions = TheGraph.merge(
            TheGraph.config.graph.edgePreview,
            edgePreviewOptions
          );
          var edgePreviewView = TheGraph.factories.graph.createGraphEdgePreview.call(
            this,
            edgePreviewOptions
          );
        ...
        var groupsGroup = TheGraph.factories.graph.createGraphGroupsGroup.call(
          this,
          groupsOptions
        );
        ...
        var edgesGroup = TheGraph.factories.graph.createGraphEdgesGroup.call(
          this,
          edgesOptions
        );
        ...
        var iipsGroup = TheGraph.factories.graph.createGraphIIPGroup.call(
          this,
          iipsOptions
        );
        ...
        var nodesGroup = TheGraph.factories.graph.createGraphNodesGroup.call(
          this,
          nodesOptions
        );

        var inportsOptions = TheGraph.merge(
          TheGraph.config.graph.inportsGroup,
          { children: inports }
        );
        var inportsGroup = TheGraph.factories.graph.createGraphInportsGroup.call(
          this,
          inportsOptions
        );

        var outportsOptions = TheGraph.merge(
          TheGraph.config.graph.outportsGroup,
          { children: outports }
        );
        var outportsGroup = TheGraph.factories.graph.createGraphGroupsGroup.call(
          this,
          outportsOptions
        );
        ...
        return TheGraph.factories.graph.createGraphContainerGroup.call(
          this,
          containerOptions,
          containerContents
        );
